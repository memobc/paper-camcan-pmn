---
title: Posterior medial network dynamics, CamCan movie watching - Discovery Sample
author: Rose Cooper
date: "`r Sys.Date()`"
output:
  html_document:
...
#bind data and add column for subject ID, as well as video and podcast names
dwiData = do.call(abind, 
                  c(lapply(dwi_files, function(x) { data = read.csv(
                                                           paste('../../dwi-data/HOA-connectomes/',x,sep=""), 
                                                           header = F,sep=" ")
                                                           if (nrow(data) == 470) { #if last row/col of 471 matrix is empty, that ROI was unconnected to add all 0s
                                                             data[471,] <- 0
                                                             data[,471] <- 0
                                                           }
                                                           return(data) 
...
# weights are interpreted as distances, so we need to reverse the streamlines -- lower (non-zero) == closer
# scale so 0 = no connection and 1 = furthest away (fewest connections)
...
for (i in 1:length(dwi_files)) {
  ...
  # create distance matrix from binarized connections
...
### store for later func-struc correlations:
dwi.pmn.distances <- melt(pm.dist.mat)
...
plot.B <- ggplot(TV.connectivity.mean) +
  ...
  scale_x_continuous(expand=c(0,0,0,0)) +
  labs(x="Time (seconds)", y='Mean Connectivity', tag="d") +
  theme(axis.line.x = element_line(color = "black", size = 1),
        axis.line.y = element_line(color = "black", size = 1),
        axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 18),
...
Is the change in functional connectivity at event transitions facilitated by or inhibited by the structural integration of that ROI with the network (weighted shortest path)?  
```{r}

colnames(event.pmn.connectivity)[4] <- 'Beta'
colnames(dwi.pmn.distances)[4] <- 'Path'

# now rename subject ids with actual strings to match
event.pmn.connectivity$Var3 <- subjects[event.pmn.connectivity$Var3]
dwi.pmn.distances$Var3 <- dwi_subjs[dwi.pmn.distances$Var3]

struc.func <- merge(dwi.pmn.distances, event.pmn.connectivity)


# now average by Module and correlate (focusing on Ventral subsystem that actually shows an increase in event-transition connectivity):
struc.func.module <- struc.func %>% group_by(Var3, Module) %>%
                      summarise(EventBeta = mean(Beta),
                                PathLength = mean(Path)) %>%
                      subset(Module != "Dorsal") %>% droplevels()
struc.func.cor <- struc.func.module %>% group_by(Module) %>%
                      summarise(R = cor.test(EventBeta, PathLength)$estimate,
                                p = p.adjust(cor.test(EventBeta, PathLength)$p.value),
                                df = cor.test(EventBeta, PathLength)$parameter)
kable(struc.func.cor, 
      caption ='Correlations between change in functional connectivity at event-transitions and average structural path length (bonferroni-corrected)')



### now add correlation plots:
plots = list()
plot.modules <- unique(struc.func.module$Module)
for (r in 1:nrow(struc.func.cor)) {
  
  data = subset(struc.func.module, Module == plot.modules[r]) %>% 
                     droplevels()

  plots[[r]] <- ggplot(data, 
                       aes(x=EventBeta, y=PathLength)) +
    geom_point(color=module.colors[r+1], size=3, alpha=.8) +
    ggtitle(plot.modules[r]) +
    geom_smooth(method = "lm", color="black", size=1.5) +
    labs(y="Structural Path Length", x="Event-Transition Beta") +
    theme(plot.title = element_text(hjust = 0.5, size=22, face="plain", margin=margin(0,0,20,0)),
          ...
          panel.background = element_blank(),
          legend.position="none",
          plot.margin = margin(0, 1.5, 0.2, 1.5, "cm"))
}

plot.C <- ggarrange(plotlist = plots, ncol=2) +
        theme(plot.tag = element_text(size = 32, face = "bold"),
              plot.margin = margin(0, 0.2, 0.4, 0.2, "cm")) +
        labs(tag="c") +
        geom_label(x=0.41, y=0.85, size=5,
              label = paste('r = ',round(struc.func.cor$R[1],3),sep="")) +
        geom_label(x=0.9, y=0.85, size=5,
              label = paste('r = ',round(struc.func.cor$R[2],3),sep=""))
...
myPlot <- ggarrange(plot.A,plot.C,plot.B, ncol = 1, nrow=3, heights=c(1.4,1.2,1))
plot(myPlot)
