...
# add module IDs (within-Ventral, within-Core, Ventral-Core)
...
p2 <- ggplot(data.summary.subject, aes(x=Module, y=Strength, color=Module)) +
  ...
  geom_hline(yintercept = 0, size = 1) +
  scale_y_continuous(expand = c(0.02,0.02,0.02,0.02), limits=c(-.4,.75)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7)) +
  labs(y='Beta', tag="b") +
  theme(axis.line.x = element_line(color = "black", size = 1), axis.line.y = element_line(color = "black", size = 1),
...
* This analysis uses a sliding-window of 25 TRs (2.47s TR, ~ 60s) to estimate a connectivity matrix at each time point, per subject. Below, I'm first plotting the mean bivariate connectivity of the Ventral, Core, and Ventral-to-Core connections over time.  
...
for (s in 1:NSubs) {
  ...
  for (t in 1:nTime) {
    ...
    sub_cors$Module = ''
    sub_cors$Module[sub_cors$Var1 %in% rois[my.modules==1] & sub_cors$Var2 %in% rois[my.modules==1]] <- 'Core'
    ...
    sub_cors$Module[(sub_cors$Var1 %in% rois[my.modules==1] & sub_cors$Var2 %in% rois[my.modules==2]) |
                      (sub_cors$Var1 %in% rois[my.modules==2] & sub_cors$Var2 %in% rois[my.modules==1])] <- 'Ventral-Core'
...
# Core
TV.subset.Core <- data.frame(subset(TV.module.connectivity, Module == "Core") %>%
                      spread(Subject, Conn))
subj.cols <- 3:ncol(TV.subset.Core)
for (s in 1:length(subjects)) {
  this.subj   <- TV.subset.Core[,subj.cols[s]]
  other.subjs <- rowMeans(TV.subset.Core[,subj.cols[-s]])
  TV.cors$Core[s] <- fisherz(cor(this.subj, other.subjs))
}
...
# Ventral-to-Core (both directions) - 
# does time-varying connectivity of the Core subsystem correlate with time-varying connectivity of the Ventral subsystem across subjects? 
for (s in 1:length(subjects)) {
  this.subj   <- TV.subset.Ventral[,subj.cols[s]]
  other.subjs <- rowMeans(TV.subset.Core[,subj.cols[-s]])
  ...
  TV.cors$`Ventral-Core`[s] <- (cor.a + cor.b)/2
}
...
ggplot(TV.cors, aes(x=fisherz2r(Z), fill=Type, color=Type, group=Type)) +
  ...
  geom_vline(data=cor.means, aes(xintercept = Mean[Type=="Core"]), linetype=2, size=1, color=module.colors[1]) +
  geom_vline(data=cor.means, aes(xintercept = Mean[Type=="Ventral"]), linetype=2, size=1, color=module.colors[2]) +
  geom_vline(data=cor.means, aes(xintercept = Mean[Type=="Ventral-Core"]), linetype=2, size=1, color=module.colors[3]) +
