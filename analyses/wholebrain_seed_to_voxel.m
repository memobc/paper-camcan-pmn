...
% for each seed (ROI) per subject, as well as a group-averaged r map.
%
...
pm_labels = spm_read_vols(spm_vol([b.ROIdir 'rPM_voxel_clusters.nii']));
...
for s = 1:length(subjs)
    ...
    for r = 1:size(roi_names,1)
        ...
        roi_data = node_data.samples(:,pm_labels(:) == roi_names{r,2});
