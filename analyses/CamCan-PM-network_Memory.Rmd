...
module.colors <- c("#fcd841","#3c7ff5","#8ce9f7")  #for "Core", "Ventral", "Ventral-Core"
...
Now, I am loading in the time-varying connectivity vectors generated within CamCan-PM-network_[Discovery/Replication].Rmd, which reflect the mean connectivity within the Ventral and Core subsystems, and between them, per subject and time point.  
```{r}
...
tv.plot <- ggplot(TV.summary) +
  ...
  scale_y_continuous(expand=c(0,0,0,0), limits=c(.15,.55)) +
...
Plots for High vs. Low memory subjects for Core and Ventral-to-Core time-varying connectivity. No visible difference between subjects, in line with the non-significant relationship between intersubject similarity of time-varying connectivity and intersubject similarity of episodic memory.  
...
core.plot <- ggplot(TV.summary) +
  ...
  geom_ribbon(alpha=0.3, aes(x=Time*my.TR, fill=Group,
                             ymin=Mean-SE, ymax=Mean+SE), color=NA) +
  geom_line(aes(x=Time*my.TR, y=Mean, color=Group), size=1.5) +
  scale_y_continuous(expand=c(0,0,0,0), limits=c(.05,.55)) +
  scale_x_continuous(expand = c(0,0,0,0)) +
  guides(fill=FALSE) +
  ggtitle("Core: High vs. Low Memory") +
...
cv.plot <- ggplot(TV.summary) +
  ...
  scale_x_continuous(expand = c(0,0,0,0)) +
  guides(fill=FALSE) +
  ggtitle("Ventral-Core: High vs. Low Memory") +
  labs(x="Time (seconds)", y='Mean Connectivity', color="Memory") +
  theme(plot.title = element_text(hjust = 0.5, size=24, margin=margin(0,0,30,0), face="plain"), 
        axis.line.x = element_line(color = "black", size = 1),
        axis.line.y = element_line(color = "black", size = 1),
...
allData$Module[allData$Node == "Hipp" | allData$Node == "MPFC" | allData$Node == "PCC" |
               allData$Node == "Prec" | allData$Node == "aAG"] <- "Core"
